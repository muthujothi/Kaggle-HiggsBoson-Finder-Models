...
train_data = pd.read_csv('C:/LearningMaterials/Kaggle/HiggsBoson/training_jet3.csv')
x_train = train_data.values[:, 1:30] #.values will convert the dataframe into a numpy array and in that use the two dimensional python slice notation
y_train = train_data.values[:, 31]
...
pcut = np.percentile(prob_predict_train,93)
print pcut

print "starting in test data"
data = pd.read_csv("C:/LearningMaterials/Kaggle/HiggsBoson/test_jet3.csv")
...
df.to_csv("predictions_jet3.csv", index=False, cols=["EventId", "Class", "SignalProb"])
