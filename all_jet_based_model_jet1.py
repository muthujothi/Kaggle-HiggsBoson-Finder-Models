...
train_data = pd.read_csv('C:/LearningMaterials/Kaggle/HiggsBoson/training_jet1.csv')
x_train = train_data.values[:, 1:23] #.values will convert the dataframe into a numpy array and in that use the two dimensional python slice notation
...
pcut = np.percentile(prob_predict_train,83)
print pcut

print "starting in test data"
data = pd.read_csv("C:/LearningMaterials/Kaggle/HiggsBoson/test_jet1.csv")
...
df.to_csv("predictions_jet1.csv", index=False, cols=["EventId", "Class", "SignalProb"])
